...
// GET /api/subscriptions/all   → { success, subscriptions[] }
// Each subscription has: clinicId (populated), planId (populated), interval,
//   status, expiryDate, paidAmount, totalPaid, appointmentsUsed, patientLimit,
//   renewalOrders[], createdAt
...
import {
  ...
  ArrowUpRight, Stethoscope, Check, AlertCircle,
...
export default function SubscriptionsPage() {
  ...
  // ── KPI Stats ──────────────────────────────────────────────────────────
  const kpi = useMemo(() => {
    let active = 0, expiring = 0, expired = 0, monthly = 0, yearly = 0, revenue = 0;
    subs.forEach((s) => {
      const m = statusMeta(s);
      if (m.label === "Active")   active++;
      if (m.label === "Expiring") expiring++;
      if (m.label === "Expired")  expired++;
      if (s.interval === "monthly") monthly++;
      if (s.interval === "yearly")  yearly++;
      ...
      <div className="sp-page">
        ...
        <div className="sp-card">
          <table className="sp-table">
            ...
            <tbody>
              {loading && (
                <tr><td colSpan={10} className="sp-center">
              ...
              {!loading && pageRows.length === 0 && (
                <tr><td colSpan={10} className="sp-center">
                  ...
                  <div>Koi record nahi mila. Filter ya search change karo.</div>
                </td></tr>
              )}
              {!loading && pageRows.map((s, i) => {
                ...
                return (
                  <tr key={s._id} onClick={() => setSelected(s)}>
                    ...
                    <td className="sp-td">
                      <div className="sp-muted">{fmt(s.createdAt)}</div>
                    </td>
                    ...
                    <td className="sp-td">
                      <div style={{ fontWeight: 800, color: "#0B2550", fontSize: "0.85rem" }}>
                        {fmtMoney(s.totalPaid)}
                      </div>
                      <div style={{ fontSize: "0.72rem", color: "#94a3b8", marginTop: "2px" }}>
                        Last: {fmtMoney(s.paidAmount)}
                      </div>
                    </td>
